...
(function () {
  ...
  function renderConcept(view){
    ...
    view.innerHTML='<div class="topic-screen"><div class="topic-content">'+
      UI.renderBreadcrumb([{label:'Início',href:''},{label:'Autovalores e Autovetores'}])+
      UI.renderPhaseBar(TOPIC_ID,'concept')+
      '<h1 class="topic-title">Autovalores e Autovetores</h1>'+
      '<p class="topic-meta">polinômio característico · diagonalização · multiplicidade · aplicações</p>'+
      '<div class="content-block">'+'<p>Autovalores λ e autovetores v satisfazem Av=λv. Revelam a estrutura intrínseca de uma transformação linear.</p>'+'<div class="concept-highlight"><div class="hl-label">Cálculo</div>1. Polinômio característico: det(A−λI) = 0 → raízes são autovalores.<br>2. Para cada λ: resolver (A−λI)v=0 → autovetores.<br>Traço = Σλᵢ | Determinante = Πλᵢ</div>'+'<div class="concept-highlight"><div class="hl-label">Diagonalização</div>A = PDP⁻¹, onde D=diag(λ₁,...,λₙ), P=[v₁|...|vₙ].<br>Possível ⟺ A tem n autovetores LI.<br>Aᵏ = PDᵏP⁻¹ (muito mais eficiente).</div>'+'<div class="concept-highlight"><div class="hl-label">Multiplicidade</div>Algébrica: multiplicidade de λ como raiz de det(A−λI)=0.<br>Geométrica: dim(Ker(A−λI)). Geom ≤ Algeb.</div>'+'</div>'+
      '<div class="btn-row mt-24"><button class="btn btn-primary" onclick="Router.navigate(\'topic/eigenvalues_adv/example\')">Ver exemplo →</button></div>'+
    '</div><div class="topic-canvas-panel" id="canvas-panel"></div></div>';
    _mc(view.querySelector('#canvas-panel'),420,380);_drawMatrix();
  }

  var _exSteps=[{"equation":"A = [[3,1],[1,3]]. Encontre autovalores e autovetores.","note":"problema"},{"equation":"det(A−λI) = (3−λ)²−1 = λ²−6λ+8 = (λ−4)(λ−2) = 0","note":"polinômio característico"},{"equation":"λ₁=4, λ₂=2.","note":"autovalores"},{"equation":"λ₁=4: (A−4I)v=0 → [−1,1;1,−1]v=0 → v₁=(1,1)","note":"autovetor para λ=4"},{"equation":"λ₂=2: (A−2I)v=0 → [1,1;1,1]v=0 → v₂=(1,−1)","note":"autovetor para λ=2"},{"equation":"P=[[1,1],[1,−1]], D=[[4,0],[0,2]], A=PDP⁻¹.","note":"diagonalização"}];
  function renderExample(view){
    _exStep=0;
    view.innerHTML='<div class="topic-screen"><div class="topic-content">'+
      UI.renderBreadcrumb([{label:'Início',href:''},{label:'Autovalores e Autovetores',href:'topic/eigenvalues_adv/concept'},{label:'Exemplo'}])+
      UI.renderPhaseBar(TOPIC_ID,'example')+
      '<h1 class="topic-title">Exemplo: autovalores e diagonalização 2×2</h1>'+
      '<p class="topic-meta">A = [[3,1],[1,3]]</p>'+
      ...
      '<div class="btn-row"><button class="btn" id="btn-prev" onclick="Eig.prevStep()" disabled>← Anterior</button>'+
      '<button class="btn btn-primary" id="btn-next" onclick="Eig.nextStep()">Próximo →</button></div>'+
    '</div><div class="topic-canvas-panel" id="canvas-panel"></div></div>';
  ...
  function _updateEx(){
    ...
    if(next){if(i===n-1){next.textContent='Praticar →';next.onclick=function(){Progress.markExample(TOPIC_ID);Router.navigate('topic/eigenvalues_adv/practice');};}    else{next.textContent='Próximo →';next.onclick=function(){Eig.nextStep();};}}
    _drawMatrix();
  ...
  function renderPractice(view){
    ...
    view.innerHTML='<div class="topic-screen"><div class="topic-content">'+
      UI.renderBreadcrumb([{label:'Início',href:''},{label:'Autovalores e Autovetores',href:'topic/eigenvalues_adv/concept'},{label:'Prática'}])+
      UI.renderPhaseBar(TOPIC_ID,'practice')+
      '<div class="practice-header"><span class="exercise-counter" id="ex-counter">Exercício 1</span>'+
      '<label class="hint-toggle" onclick="Eig.toggleHints()">Dicas<div class="toggle-switch" id="hint-toggle-sw"><div class="toggle-knob"></div></div></label></div>'+
  ...
  function _renderCard(){
    ...
    area.innerHTML='<div class="exercise-card">'+
      ...
      '<div class="btn-row"><button class="btn btn-primary" onclick="Eig.checkAnswer()">Verificar</button>'+
      '<button class="btn" id="btn-hint" onclick="Eig.showNextHint()" style="display:none">Ver dica</button>'+
      '<button class="btn" id="btn-next-ex" onclick="Eig.nextExercise()" style="display:none">Próximo →</button></div></div>';
    var inp=document.getElementById('answer-input');
    if(inp){inp.addEventListener('keydown',function(e){if(e.key==='Enter')Eig.checkAnswer();});inp.focus();}
    _uhb();
  }
  function _uhb(){var b=document.getElementById('btn-hint');if(b)b.style.display=_pr.hintsEnabled&&!_pr.solved?'':'none';}

  var _pub={
    nextStep:function(){if(_exStep<_exSteps.length-1){_exStep++;_updateEx();}else{Progress.markExample(TOPIC_ID);Router.navigate('topic/eigenvalues_adv/practice');}},
